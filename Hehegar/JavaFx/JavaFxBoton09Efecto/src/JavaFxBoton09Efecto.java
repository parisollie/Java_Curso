...
import javafx.scene.effect.DropShadow;//Agrega sombras
import javafx.scene.effect.Reflection;//Agrega reflexion
import javafx.scene.effect.Lighting;//Efecto de liz
import javafx.scene.input.MouseEvent;
public class JavaFxBoton09Efecto extends Application  {
    ...
    public void efectosyEstilo(){
        sombra = new DropShadow();
        reflexion = new Reflection();
        iluminacion = new Lighting();
        btnNuevo.addEventHandler(MouseEvent.MOUSE_ENTERED,evento-> btnNuevo.setEffect(reflexion));
        //Cuando pasen por encima el puntero del mouse se hara
        btnNuevo.addEventHandler(MouseEvent.MOUSE_EXITED,evento-> btnNuevo.setEffect(null)); // le quitamos la sombra con null
        
        btnGuardar.addEventHandler(MouseEvent.MOUSE_ENTERED,evento-> btnGuardar.setEffect(sombra));
        //Cuando pasen por encima el puntero del mouse se hara
        btnGuardar.addEventHandler(MouseEvent.MOUSE_EXITED,evento-> btnGuardar.setEffect(null)); // le quitamos la reflexion con null
        
        btnCancelar.addEventHandler(MouseEvent.MOUSE_ENTERED,evento-> btnCancelar.setEffect(iluminacion));
        //Cuando pasen por encima el puntero del mouse se hara
        btnCancelar.addEventHandler(MouseEvent.MOUSE_EXITED,evento-> btnCancelar.setEffect(null)); 
        
        //Cambiando el color del boton y la fuente
        btnNuevo.setStyle("-fx-font:12 ComicSam; -fx-base: #32CD99");
        //Cambiando el color del boton y la fuente
        btnGuardar.setStyle("-fx-font:12 ComicSam; -fx-base: #9370DB");
        
        btnCancelar.setStyle("-fx-font:12 ComicSam; -fx-base: #70DBDB");
